  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="90px" class="ui-filter-card">
      ...
      <el-form-item label="订单号" prop="outTradeNo">
        <el-input v-model="queryParams.outTradeNo" placeholder="请输入订单号" clearable style="width: 220px" @keyup.enter="handleQuery" />
      </el-form-item>
      <el-form-item label="发票号码" prop="invoiceNo">
        <el-input v-model="queryParams.invoiceNo" placeholder="请输入发票号码" clearable style="width: 200px" @keyup.enter="handleQuery" />
      </el-form-item>
    ...
    <div class="ui-table-card">
      <el-table v-loading="loading" :data="invoiceList">
        <el-table-column label="发票号码" prop="invoiceNo" min-width="150" :show-overflow-tooltip="true" />
        <el-table-column label="发票代码" prop="invoiceCode" min-width="130" :show-overflow-tooltip="true" />
        ...
        <el-table-column label="开票时间" align="center" prop="invoiceIssueTime" width="180">
          <template #default="scope">{{ parseTime(scope.row.invoiceIssueTime) || '-' }}</template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="View" @click="handleDetail(scope.row)" v-hasPermi="['biz:invoice:query']">详情</el-button>
            <el-button v-if="scope.row.invoicePdfUrl" link type="primary" icon="Download" @click="openPdf(scope.row.invoicePdfUrl)">PDF</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-drawer v-model="detailOpen" title="发票详情" size="680px" append-to-body>
...
<script setup name="PiiInvoice">
import { listInvoice, getInvoice } from '@/api/pii/invoice'
...
const data = reactive({
  queryParams: {
    ...
    invoiceIssueTimeBegin: undefined,
    invoiceIssueTimeEnd: undefined
  }
})
...
function syncIssueTimeRange() {
  queryParams.value.invoiceIssueTimeBegin = issueTimeRange.value && issueTimeRange.value.length === 2 ? issueTimeRange.value[0] : undefined
  queryParams.value.invoiceIssueTimeEnd = issueTimeRange.value && issueTimeRange.value.length === 2 ? issueTimeRange.value[1] : undefined
}
