  ...
  <div class="app-container ui-list-page task-page">
    ...
    <section class="task-workspace">
      <div class="ui-action-bar">
        <a-space>
          <a-button
            v-hasPermi="['monitor:importtask:submit']"
            type="primary"
            @click="openSubmit"
          >
            <template #icon><icon-upload /></template>新建导入任务
          </a-button>
      ...
      <div class="ui-table-card">
        ...
        >
          <template #columns>
            ...
            <a-table-column title="成功/失败" :width="130">
              <template #cell="{ record }">
                <span class="success-count">{{ record.successCount || 0 }}</span>
                <span class="count-separator">/</span>
                <span class="failure-count">{{ record.failureCount || 0 }}</span>
              </template>
            </a-table-column>
              ...
              <template #cell="{ record }">
                <a-space class="table-action-buttons">
                  <a-tooltip content="查看详情">
                    <a-button
                      v-hasPermi="['monitor:importtask:query']"
                      type="text"
                  ...
                  <a-tooltip content="取消任务">
                    <a-button
                      v-hasPermi="['monitor:importtask:cancel']"
                      type="text"
                      status="danger"
                      class="table-action-button"
                      aria-label="取消任务"
    ...
    <a-modal
      ...
      @ok="submitTask"
    >
      <a-form class="modal-form" :model="submitForm" :label-col-props="{ span: 6 }">
        ...
        <a-form-item label="覆盖已有用户">
          <a-switch v-model="submitForm.updateSupport" />
        </a-form-item>
      </a-form>
    </a-modal>

    <a-modal v-model:visible="detailVisible" title="导入任务详情" :footer="false" :width="640">
...
<script lang="ts" setup>
  ...
  const submitForm = reactive({ updateSupport: false });
  ...
  async function loadData() {
    loading.value = true;
    try {
      const response = await listImportTasks({
        ...query,
        pageNum: page.value,
  ...
  function openSubmit() {
    fileList.value = [];
    submitForm.updateSupport = false;
    submitVisible.value = true;
  }

  function handleFileChange(items: FileItem[]) {
  ...
  async function submitTask() {
    ...
    try {
      await submitImportTask(file, submitForm.updateSupport);
  ...
  function cancelTask(record: TaskRecord) {
    Modal.confirm({
      ...
      onOk: async () => {
        await cancelImportTask(record.taskId);
