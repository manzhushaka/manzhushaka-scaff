...
<script lang="ts" setup>
  ...
  const fetchData = async (params: PublicOpinionAnalysis) => {
    try {
      const response = await queryPublicOpinionAnalysis(params);
      ...
      if (props.chartType === 'bar') {
        ...
        lineData.value[0] = chartData.map((el) => el.y);
        lineData.value[1] = chartData.map((el) => el.y);
        chartOption.value = lineChartOption.value;
      } else {
        chartData.forEach((el) => {
          pieData.value.push({ value: el.y, name: el.name });
        });
