...
public class AsyncFactory
{
     ...
     * 慢 SQL 日志记录。
     *
     * @param slowSqlLog 慢 SQL 日志信息
     * @return 任务 task
     */
    public static TimerTask recordSlowSql(final SlowSqlLogRecord slowSqlLog)
    {
        return new TimerTask()
        {
            @Override
            public void run()
            {
                SpringUtils.getBean(SystemAuditAppService.class).recordSlowSqlLog(
                        slowSqlLog.mapperId(), slowSqlLog.sqlText(), slowSqlLog.dataSourceName(),
                        slowSqlLog.costTime(), slowSqlLog.errorMsg(), slowSqlLog.executeTime());
            }
        };
    }
}
